   ...
   environment variable. 
	...
	A. Locate the name of the environment variable to set, using the 
	 ...
	 NOTE: When deploying C and C++ standalone applications, it is possible
               to run the shell script file run_sys_import.sh on UNIX and Mac
...
3. Create packages that contain the following files, based on what
   you are deploying: 
	...
	All Platforms:
		* sys_import.ctf (Component Technology File archive,
		  platform-dependent on target machine)
			
	Windows:
		* MCRInstaller.exe (self-extracting MATLAB Component Runtime
		  library archive)  
		* sys_import.exe (application created by MATLAB Compiler)
	UNIX:
		* MCRInstaller.zip (MATLAB Component Runtime library 
	 	  archive - manual unzip required)
		* sys_import (application created by MATLAB Compiler)
		* run_sys_import.sh (shell script that can be run 
	...
	All Platforms:            
		* sys_import.dll (shared library, file extension 
		  varies by platform)
		* sys_import.h (library header file)
		* sys_import.ctf (Component Technology File archive,
		  platform-dependent on target machine)
	...
	* sys_import.ctf (Component Technology File archive)	
	* sys_import.xml (documentation files)
	* sys_import.pdb (program database file - if 
	  DEBUG option selected)
	* sys_import.dll (component assembly file)
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  
	...
	* sys_import.ctf (Component Technology File archive)
	* sys_import_1_0.dll (component containing 
	  compiled M-code)
	...
	* sys_import.jar (Java archive containing Java 
	  interface to M-code in sys_import.ctf)
	* sys_import.ctf (Component Technology File archive,
	  platform-dependent on target machine)
