...
%
% Outputs:
%    D - Polynomial matrix in sym format. Left coprime Fraction 
%    N - Polynomial matrix in sym format. Left coprime Fraction  
%    G=D^(-1)N 
%
% Example: 
...
if sum(v)~=l %no es minima si es igual
    ...
    if fg==cg
        [Do No]=mtf2rmpf(G);
        cn=length(No);
        [S dep lidep]=mSilvester(Do,No,0,'ascend');
        S=S';        
        cs=length(S);
        dep1=dep;
        [fdp cdp]=size(dep);
        if ~isempty(dep)
            ...
            [A B C]=rmpf2ss(Do,No);
            [v Q Af Bf Cf]=canonform(A,B,C,4);
            sys=ss(Af,Bf,Cf,zeros(fg,cg));
            [P Q]=ss2lmpf(sys);            
            [D N]=sym2cell(P,Q);
        end
    else
        warndlg('In the moment lcf is posible only square MTF','!! Warning !!')
    end
end
[D N]=cellround(D,N);
[D N]=cell2sym(D,N);